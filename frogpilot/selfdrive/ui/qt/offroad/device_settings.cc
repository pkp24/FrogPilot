...
FrogPilotDevicePanel::FrogPilotDevicePanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> deviceToggles {
    {"DeviceManagement", tr("Device Settings"), tr("<b>Change how the device powers off, handles heat, and records your drives.</b>"), "../../frogpilot/selfdrive/assets/offroad/icon_device.png"},
    ...
    {"ScreenManagement", tr("Screen Settings"), tr("<b>Change how bright the screen is, how long it stays on, and whether you can record it.</b>"), "../../frogpilot/selfdrive/assets/offroad/icon_light.png"},
  ...
  for (const auto &[param, title, desc, icon] : deviceToggles) {
    AbstractControl *deviceToggle = nullptr;
    ...
    if (param == "DeviceManagement") {
      ...
      QObject::connect(screenRecorder(), &ScreenRecorder::stateChanged, recorderToggle, updateRecorderToggle);
      updateRecorderToggle();
