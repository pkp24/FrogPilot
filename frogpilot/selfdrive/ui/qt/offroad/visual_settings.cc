...
QMap<int, QString> FrogPilotVisualsPanel::developerMetricOptions() {
  return {
    {0, tr("None")},
...
QMap<int, QString> FrogPilotVisualsPanel::mapStyleOptions() {
  return {
...
FrogPilotVisualsPanel::FrogPilotVisualsPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> visualToggles {
    {"AdvancedCustomUI", tr("Advanced UI Controls"), tr("<b>Fine-tune how the driving screen looks, beyond what the everyday settings cover.</b><br><br>None of these change how the car drives."), "../../frogpilot/selfdrive/assets/offroad/icon_advanced_device.png"},
    ...
    {"ModelUI", tr("Model UI"), tr("<b>Change how openpilot draws the road ahead, including the driving path, the lane lines and the road edges.</b><br><br>Turning this off falls back to stock openpilot's sizes, though the coloured path edges go away with it."), "../../frogpilot/selfdrive/assets/offroad/icon_road.png"},
    ...
    {"NavigationUI", tr("Navigation Widgets"), tr("<b>Change what navigation shows on the driving screen, from the map itself to speed limit signs.</b><br><br>Some of these need map data downloaded under \"Maps and Navigation\" before they show anything."), "../../frogpilot/selfdrive/assets/offroad/icon_map.png"},
    ...
    {"QOLVisuals", tr("Quality of Life"), tr("<b>Pick which camera you watch, and a couple of smaller driving screen touches that did not fit anywhere else.</b>"), "../../frogpilot/selfdrive/assets/offroad/icon_quality_of_life.png"},
  ...
  for (const auto &[param, title, desc, icon] : visualToggles) {
    AbstractControl *visualToggle = nullptr;
    ...
    if (param == "AdvancedCustomUI") {
      ...
      QMap<int, QString> styleMap = mapStyleOptions();
...
void FrogPilotVisualsPanel::showEvent(QShowEvent *event) {
  ...
  static_cast<ButtonControl*>(toggles["MapStyle"])->setValue(mapStyleOptions().value(params.getInt("MapStyle")));
