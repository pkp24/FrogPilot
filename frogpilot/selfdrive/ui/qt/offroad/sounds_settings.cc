...
void FrogPilotSoundsPanel::playSound(const QString &alert, int volume) {
  QString stockPath = "../../selfdrive/assets/sounds/" + alert + ".wav";
  QString themePath = "../../frogpilot/selfdrive/assets/active_theme/sounds/" + alert + ".wav";
  ...
  soundPlayerProcess->start("ffplay", QStringList{"-nodisp", "-autoexit", "-volume", QString::number(clampedVolume), filePath});
}
...
FrogPilotSoundsPanel::FrogPilotSoundsPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  for (const auto &[param, title, desc, icon] : soundsToggles) {
    AbstractControl *soundsToggle = nullptr;
