...
FrogPilotLateralPanel::FrogPilotLateralPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> lateralToggles {
    {"AdvancedLateralTune", tr("Advanced Lateral Tuning"), tr("<b>Hand-set the steering numbers openpilot normally works out for itself, and switch that learning on or off.</b><br><br>Wrong values show up as a wheel that feels twitchy or lazy. Every number has a \"Reset\" button that puts your car's original value back."), "../../frogpilot/selfdrive/assets/offroad/icon_advanced_lateral_tune.png"},
    ...
    {"LaneChanges", tr("Lane Changes"), tr("<b>Allow openpilot to change lanes.</b>"), "../../frogpilot/selfdrive/assets/offroad/icon_lane.png"},
    ...
    {"LateralTune", tr("Lateral Tuning"), tr("<b>Switch openpilot's steering over to a neural network for a smoother wheel, and have it steer into turns when you signal below your minimum lane change speed.</b>"), "../../frogpilot/selfdrive/assets/offroad/icon_lateral_tune.png"},
    ...
    {"QOLLateral", tr("Quality of Life"), tr("<b>Hand the wheel back to yourself below a speed you set.</b>"), "../../frogpilot/selfdrive/assets/offroad/icon_quality_of_life.png"},
  ...
  for (const auto &[param, title, desc, icon] : lateralToggles) {
    AbstractControl *lateralToggle = nullptr;
    ...
    if (param == "AdvancedLateralTune") {
      ...
      lateralToggle = new FrogPilotParamValueButtonControl(param, title, desc, icon, 0.01, 0.5, QString(), std::map<float, QString>(), 0.01, false, {}, steerFrictionButton, false, false);
...
void FrogPilotLateralPanel::updateToggles() {
  for (auto &[key, toggle] : toggles) {
    if (parentKeys.contains(key)) {
      toggle->setVisible(parent->tuningLevel >= frogpilotToggleLevels[key].toDouble());
    }
  }
