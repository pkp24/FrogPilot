...
#include "frogpilot/selfdrive/ui/qt/offroad/device_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/frogpilot_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/lateral_settings.h"
...
#include "frogpilot/selfdrive/ui/qt/offroad/model_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/navigation_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/sounds_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/theme_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/utilities.h"
#include "frogpilot/selfdrive/ui/qt/offroad/vehicle_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/visual_settings.h"
#include "frogpilot/selfdrive/ui/qt/offroad/wheel_settings.h"
...
void FrogPilotSettingsWindow::createPanelButtons(FrogPilotListWidget *list) {
  ...
  std::vector<std::tuple<QString, QString, QString>> panelInfo = {
    {tr("Alerts and Sounds"), tr("<b>Set the volume for each of openpilot's alerts, and add extra alerts stock openpilot doesn't have.</b> Extra alerts include a chime when the light turns green or when the car ahead starts moving."), "../../frogpilot/selfdrive/assets/offroad/icon_sound.png"},
    {tr("Driving Controls"), tr("<b>Adjust how openpilot accelerates, brakes, steers, and changes lanes, and switch between driving models.</b><br><br>\"DRIVING MODEL\" only appears once your \"Tuning Level\" is \"Standard\" or higher, and \"GAS / BRAKE\" only appears on cars where openpilot handles the gas and brake."), "../../frogpilot/selfdrive/assets/offroad/icon_steering.png"},
    {tr("Maps and Navigation"), tr("<b>Download the speed limit data openpilot uses, and set up turn-by-turn navigation.</b> Speed limits come from offline map data for the states or countries you pick, so they work without cell signal."), "../../frogpilot/selfdrive/assets/offroad/icon_map.png"},
    {tr("System Settings"), tr("<b>Manage your saved data, how the device and screen behave, and tools for fixing problems.</b> This is also where your drive stats and backups of your settings live."), "../../frogpilot/selfdrive/assets/offroad/icon_system.png"},
    {tr("Theme and Appearance"), tr("<b>Change what appears on the driving screen, and how openpilot looks and sounds.</b> Anything from hiding on-screen icons to full theme packs with new colors, sounds, and turn signal animations."), "../../frogpilot/selfdrive/assets/offroad/icon_display.png"},
    {tr("Vehicle Settings"), tr("<b>Tell openpilot what car you drive, turn on features made for your brand, and change what your steering wheel buttons do.</b><br><br>Brand features include things like smoother stop-and-go and automatic door locks. \"WHEEL BUTTONS\" only appears once your \"Tuning Level\" is \"Advanced\" or higher."), "../../frogpilot/selfdrive/assets/offroad/icon_vehicle.png"}
  };
...
FrogPilotSettingsWindow::FrogPilotSettingsWindow(SettingsWindow *parent) : QFrame(parent) {
  ...
  togglePreset = new FrogPilotButtonsControl(tr("Tuning Level"),
                                             tr("<b>Choose how much control you want over FrogPilot's settings.</b> Anything above your level is hidden and uses FrogPilot's recommended setting instead. Nothing you've set is lost, and it comes back when you move up.<br><br>"
                                              ...
                                              "../../frogpilot/selfdrive/assets/offroad/icon_tuning.png", togglePresets, true);
...
void FrogPilotSettingsWindow::updateVariables() {
  ...
  if (!carParams.empty()) {
    ...
    std::string effectiveCarFingerprint = carModel.empty() ? carFingerprint : carModel;
    carMake = CP.getCarName();

    carDetected = true;
    friction = CP.getLateralTuning().which() == cereal::CarParams::LateralTuning::TORQUE ? CP.getLateralTuning().getTorque().getFriction() : 0.0;
    ...
    isVolt = effectiveCarFingerprint == "CHEVROLET_VOLT";
    latAccelFactor = isTorqueCar ? CP.getLateralTuning().getTorque().getLatAccelFactor() : 0.0;
