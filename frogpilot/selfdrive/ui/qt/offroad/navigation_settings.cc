...
FrogPilotNavigationPanel::FrogPilotNavigationPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  updateSpeedLimitsToggle = new FrogPilotButtonToggleControl("SpeedLimitFiller", tr("Speed Limit Filler"),
                                                    tr("<b>Collect missing or incorrect speed limits automatically while you drive.</b><br><br>"
                                                       ...
                                                       "FrogPilot uses their OSM way IDs to identify each road. Collected limits are reused on later drives in the same direction, "
                                                       "including after map updates, until the downloaded maps match.<br><br>You can download the results from \"The Pond\" in "
...
void FrogPilotNavigationPanel::showEvent(QShowEvent *event) {
  ...
  searchInput->setCheckedButton(selectedSearchInput);
...
void FrogPilotNavigationPanel::updateStep() {
  QString step;
  if (setupCompleted) {
    step = "../../frogpilot/selfdrive/assets/navigation/navigation_training/setup_completed.png";
  } else if (mapboxPublicKeySet && mapboxSecretKeySet) {
    step = "../../frogpilot/selfdrive/assets/navigation/navigation_training/both_keys_set.png";
  } else if (mapboxPublicKeySet) {
    step = "../../frogpilot/selfdrive/assets/navigation/navigation_training/public_key_set.png";
  } else {
    step = "../../frogpilot/selfdrive/assets/navigation/navigation_training/no_keys_set.png";
  }
